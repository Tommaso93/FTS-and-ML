...
def flatten_dict(d):
    """takes dict object and flatens it using dot notation"""  #permette di appiattire il mio dictionary
    def items():
        for key, value in d.items():
            if isinstance(value, dict): #se l'oggetto è un dizionario
                for subkey, subvalue in flatten_dict(value).items(): #per ogni subdict
                    yield key + field_delimter + subkey, subvalue #costruisci un dict con questa forma
            else:
                yield key, value #altrimenti lascialo dizionario come prima

    return dict(items())


def reorderKeyList(key_list, import_k_l):  #riordina la mia lista in base ad un insieme di chiavi importanti
...
def readFileToList(filename):
    ...
    with open(filename) as json_file:
        rec_list = []  #lista di record
        if debug is True:
            print("-----------")
        for line in json_file:
            json_dict = flatten_dict(json.loads(line.translate(None, escapes))) #caricare il json riga per riga e quindi appiattirlo.
            rec_list.append(json_dict) #aggiugere il record alla lista
    if debug is True:
        print rec_list[0]
        print "-------------"
        print rec_list[5]
...
def recordsListTransform(records_list):   #cambiare il tipo ai vari tipi di variabili
...
def jsonListToCSV(jsonList, keysSet, outputPath):  #scrivere il .csv file
...
def main(argv):
    ...
    inputfile = os.path.join(working_path, "sample.json")
    inputdir = os.path.join(working_path, "/summer_student")
